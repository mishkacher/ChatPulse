...
chrome.runtime.onInstalled.addListener(() => {
  void initializeExtension("Установлено расширение ChatPulse");
});

chrome.runtime.onStartup.addListener(() => {
  void initializeExtension("Chrome запущен: создана новая безопасная сессия наблюдения");
});
...
async function initializeExtension(logMessage) {
  ...
  state = appendLog(state, "info", logMessage);
...
async function addCurrentChat() {
  ...
  try {
    ...
    // Заголовок вкладки остаётся безопасным резервным значением.
  }

  let state = await loadState();
  const existingIndex = state.chats.findIndex((chat) => chat.url === normalizedURL);
  if (existingIndex >= 0) {
...
async function performCheck(source, allowWhenStopped) {
  ...
  await saveState(observedState);
  await updateBadge(observedState);
  await broadcastState(observedState);
  ...
  for (let index = 0; index < observedState.chats.length; index += 1) {
    ...
    try {
      ...
      observedState.chats[index] = { ...chat, lastError: message };
  ...
  observedState = await configureAlarm(observedState);

  const latestState = await loadState();
  const merged = mergeRuntimeState(observedState, latestState);
...
async function ensureChatTab(chat) {
  if (Number.isInteger(chat.tabId)) {
    try {
      ...
      // Вкладка закрыта — ищем другую или создаём новую.
    }
  }
...
async function waitForTabComplete(tabId, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const onRemoved = (removedTabId) => {
      if (removedTabId === tabId) finish(new Error("Вкладка ChatGPT была закрыта во время проверки."));
    };
...
async function updateBadge(state) {
  ...
  await chrome.action.setBadgeBackgroundColor({ color: state.checkInProgress ? "#9B5CFF" : "#2C8CFF" });
...
async function broadcastState(state) {
  try {
    ...
    // Popup/options могут быть закрыты — это штатно.
  }
}
...
function decisionDescription(decision) {
  const descriptions = {
    ...
    "not-authenticated": "в текущем профиле Chrome не выполнен вход",
