...
test("неотвечающий content script фоновой вкладки приводит к обновлению", () => {
  assert.deepEqual(
...
test("активная вкладка не перезагружается даже при недоступном content script", () => {
  assert.deepEqual(
    planTabRecovery({
      tab: tab({ active: true }),
      snapshot: null,
...
test("активная вкладка не перезагружается автоматически при ошибке страницы", () => {
  assert.deepEqual(
    planTabRecovery({
      tab: tab({ active: true }),
      snapshot: snapshot({ errorDetected: true }),
      chat: chat(),
      intervalMinutes: 5,
      now
    }),
...
test("пользовательский черновик защищён даже при ошибке страницы", () => {
  assert.deepEqual(
    planTabRecovery({
      tab: tab(),
      snapshot: snapshot({ hasDraft: true, errorDetected: true }),
      chat: chat({ lastHardRefreshAt: "2026-07-22T20:00:00.000Z" }),
      intervalMinutes: 1,
      now
    }),
